...
export default function RendirExamen() {
  ...
  // Estructura interna: { [preguntaId]: 'A' | 'B' | ... }
  ...
  const inicializarExamen = useCallback(async () => {
    try {
      ...
      // 1. Validar la postulación y obtener su ID real.
      const misPostulaciones =
        await postulacionService.listarMisPostulaciones();

      const postulacionActiva = misPostulaciones.find(
        (postulacion) =>
          postulacion.vacanteId === Number(vacanteId)
      );

      if (!postulacionActiva) {
        setError(
          'No se encontró una postulación asociada a esta vacante de empleo.'
        );
        setLoading(false);
        return;
      }
      ...
      // Guardamos el ID para utilizarlo al enviar el examen.
      setPostulacionId(postulacionActiva.id);

      // 2. Descargar el examen técnico de la vacante.
      const examenData =
      ...
      if (
        ...
        examenData.preguntas.length === 0
      ) {
        setError(
          'Esta vacante no cuenta con un banco de preguntas activo en este momento.'
        );
        setLoading(false);
        return;
      }
      ...
      // Inicializar una respuesta vacía por pregunta.
      const inicializarRespuestas = {};

      examenData.preguntas.forEach((pregunta) => {
        inicializarRespuestas[pregunta.id] = '';
      });

      setRespuestas(inicializarRespuestas);
    } catch (err) {
      console.error(
        'Error al inicializar el examen técnico:',
        err
      );

      setError(
        'Ocurrió un error al cargar la evaluación técnica. Por favor, contacta al administrador.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmitExamen = async (e) => {
    ...
    // Validar que la postulación haya sido cargada.
    if (!postulacionId) {
      alert(
        'No se pudo identificar la postulación asociada al examen.'
      );
      return;
    }
    ...
    if (preguntasSinResponder.length > 0) {
      alert(
        `Por favor, responde todas las preguntas antes de enviar. Te faltan ${preguntasSinResponder.length} por contestar.`
      );
      return;
    }
    ...
    try {
      ...
      /*
       * Convertir el mapa de respuestas al nuevo formato
       * esperado por Spring Boot.
       */
      const respuestasParaBackend =
      ...
      /*
       * Se utiliza postulacionId, que fue guardado
       * en el estado durante inicializarExamen.
       */
      await postulacionService.calificarEvaluacion(
        postulacionId,
        payload
      );

      alert(
        '¡Evaluación técnica enviada y procesada con éxito!'
      );
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24">
        ...
        <span className="mt-4 text-slate-600 font-medium">
          Cargando cuestionario técnico seguro...
        </span>
      </div>
    );
  }
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-4 w-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      >
                        <span
                          className={`w-5 h-5 rounded-full border flex items-center justify-center mr-3 text-xs font-bold ${
                            respuestas[pregunta.id] === 'A'
                      ...
                      {/* Opciones C y D solamente para preguntas múltiples */}
                      {!esVerdaderoFalso &&
                        pregunta.opcionC && (
                          <button
                            type="button"
